    ...
    if( $checkWActive ){
        ...
        if ( $donor == 'customer' &&  $donationType == 'fixed' && !empty($amounts) ) {
            ...
            if ( $amount_cents > 0 ) {
                ...
                $html .= '<button style="background: '.$text_color.'" class="radio-button ' . $selected . '" data-value="' . $amount_cents . '" data-label="' . $roundupValue . '" >' . $roundupValue . $currency_symbol . '</button>';
